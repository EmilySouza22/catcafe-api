...
test.describe('CardapioPage', () => {
	...
	test.describe('Renderização', () => {
		...
		test('deve exibir título e descrição da categoria ativa (Cafés)', async ({
			page,
		}) => {
			await expect(page.getByRole('heading', { name: 'Cafés' })).toBeVisible();
			await expect(
				page.getByText('Grãos selecionados e preparos especiais'),
			).toBeVisible();
		});
		...
		test('cards devem exibir nome, preço e descrição', async ({ page }) => {
			...
			await expect(
				page.getByText('Leve e cremoso, com espresso'),
			).toBeVisible();
		});
	});
	...
	test.describe('Destaques do Cardápio', () => {
		test('deve exibir a seção de destaques com título correto', async ({
			page,
		}) => {
			await expect(
				page.getByRole('heading', { name: 'Destaques do Cardápio' }),
			).toBeVisible();
			await expect(
				page.getByText('Combinações perfeitas para acompanhar seu café.'),
			).toBeVisible();
		});

		test('deve exibir todos os itens de destaque com nome e preço', async ({
			page,
		}) => {
	...
	test.describe('Acessibilidade', () => {
		test('todas as imagens com conteúdo devem ter alt text não vazio', async ({
			page,
		}) => {
		...
		test('abas devem ser ativáveis via tecla Enter', async ({ page }) => {
			...
			await expect(
				page.getByRole('heading', { name: 'Bebidas' }),
			).toBeVisible();
		});
